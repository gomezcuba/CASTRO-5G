...
for n in range(nClusters):
    ...
    plt.subplot(2,1,1)
    markerline, stemlines, baseline = plt.stem(subpaths.loc[n,:].tau *1e9,np.real(pathAmplitudes))
    plt.setp(stemlines, color=cm.jet(n/(nClusters-1)))
    plt.setp(markerline, color=cm.jet(n/(nClusters-1))) 
    plt.subplot(2,1,2)
    markerline, stemlines, baseline = plt.stem(subpaths.loc[n,:].tau *1e9,np.imag(pathAmplitudes))
    plt.setp(stemlines, color=cm.jet(n/(nClusters-1)))
    plt.setp(markerline, color=cm.jet(n/(nClusters-1))) 
plt.subplot(2,1,1)
...
for n in range(nClusters):
    ...
    markerline, stemlines, baseline = plt.stem(subpaths.loc[n,:].tau *1e9, pathAmpdBtrunc25, bottom=mindB)
    plt.setp(stemlines, color=cm.jet(n/(nClusters-1)))
    plt.setp(markerline, color=cm.jet(n/(nClusters-1))) 
...
for p in range(Ntop):
    ...
    pulsesOversampling = np.sinc((t-subpaths.loc[n,m].tau *1e9)/Ts)
    plt.plot(t/Ts,pathCoefAbs*pulsesOversampling,':',color=cm.jet(p/(Ntop-1)))
    
    pulses = np.sinc(taps-subpaths.loc[n,m].tau *1e9/Ts)
...
pulsesOversampling = np.sinc((t-subpaths.tau.to_numpy() *1e9)/Ts)
pulses = np.sinc(taps -subpaths.tau.to_numpy() *1e9/Ts)
