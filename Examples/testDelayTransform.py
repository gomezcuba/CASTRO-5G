...
liRX_cA = np.where(clustersAD.Xs<np.inf,np.sqrt((clustersAD.Xs-rx[0])**2+(clustersAD.Ys - rx[1])**2),25)
liRX_sA = np.where(subpathsAD.Xs<np.inf,np.sqrt((subpathsAD.Xs-rx[0])**2+(subpathsAD.Ys - rx[1])**2),25)
#Distancia entre transmisor e posicion do rebote
liTX_cA = np.where(clustersAD.Xs<np.inf,np.sqrt((clustersAD.Xs)**2+(clustersAD.Ys)**2),25)
liTX_sA = np.where(subpathsAD.Xs<np.inf,np.sqrt((subpathsAD.Xs)**2+(subpathsAD.Ys)**2),25)
