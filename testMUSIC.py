...
garray = lambda_fArray(np.array([angle]),Nant,.5)[0,:,:]
...
COVy=np.cov(yarray)
noiseEigenvectors=np.linalg.eigh(COVy)[1][:,0:-1]
signalEigenvector=np.linalg.eigh(COVy)[1][:,-1:]

theta=np.linspace(0,2*np.pi,1001)
fMUSIC=np.sum(np.abs(noiseEigenvectors.T.conj()@lambda_fArray(theta,Nant))**2,axis=(1,2))
# fMUSIC=1/np.sum(np.abs(signalEigenvector.T.conj()@lambda_fArray(theta,Nant))**2,axis=(1,2))
...
fQ=np.sum(np.abs(modangdif( yphasedif, yphasedif_expected ) )**2,axis=(1,2))

fBeamforming=(np.sum(np.abs(yarray)**2)-np.sum(np.abs(yarray.T.conj()@lambda_fArray(theta,Nant) )**2,axis=(1,2)))

#inspired by MUSIC, we estimate the covariance between antenna n-1 and n, but directly in the phases 
yphaseOfCovariance_diff = np.mean(modangdif( yphasedif, yphasedif_expected ),axis=2)
