...
plt.plot([x0_true[0],x0_true[0]+1.2*scaleguide*.05*np.shape(theta_true)[0]*np.cos(phi0_true[0])],[y0_true[0],y0_true[0]+1.2*scaleguide*.05*np.shape(theta_true)[0]*np.sin(phi0_true[0])],'k')
...
phi0_search=np.linspace(0,2*np.pi,1000)
...
for ct in range(phi0_search.size):
    for gr in range(Npath-2):
        (d0_3path[ct,gr,:],tauE_3path[ct,gr],_)= loc.computeAllPaths(AoD[gr:gr+3],AoA[gr:gr+3],dels[gr:gr+3],rotation=phi0_search[ct])
...
for ct in range(phi0_search.size):
    for gr in range(Npath):
        (d0_drop1[ct,gr,:],tauE_drop1[ct,gr],_)=loc.computeAllPaths(AoD[np.arange(Npath)!=gr],AoA[np.arange(Npath)!=gr],dels[np.arange(Npath)!=gr],rotation=phi0_search[ct])
