...
x=np.round(np.random.rand(16,1))*2-1 + 1j*(np.round(np.random.rand(16,1))*2-1)
...
yinv=ytinv[N-1:-N+1]
y=yt[N-1:-N+1]
print("Time invariant filters are equal: %s"%np.all(np.isclose(yinv,np.fft.ifft(np.fft.fft(x,16,0)*np.fft.fft(h0,16,0),16,0))))
